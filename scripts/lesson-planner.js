...
document.getElementById('model').addEventListener('change', function() {
    ...
    // Re-add buttons after table rebuild
    addSnippetButtons();
});
...
// Coursebook snippets functionality
let coursebookSnippets = {};

async function loadCoursebookSnippets() {
    const lessons = [
        { path: '../Lessons/Beginner/lesson6_listen_practice.html', title: 'Lesson 6: Listen & Practice Speaking' },
        { path: '../Lessons/Business Communication/job_interview_lesson.html', title: 'Job Interview Practice' }
    ];
    ...
    for (const lesson of lessons) {
        try {
            ...
            const activities = Array.from(doc.querySelectorAll('.activity')).map(activity => ({
                title: activity.closest('section') ? activity.closest('section').querySelector('h2')?.textContent || 'Activity' : 'Activity',
                content: activity.textContent.trim()
            }));

            coursebookSnippets[lesson.title] = activities;
        } catch (error) {
            console.error('Error loading lesson:', lesson.path, error);
        }
    }
    console.log('Coursebook snippets loaded:', coursebookSnippets);
}

function showSnippetModal(activityInput) {
    ...
    let html = '<h3 style="margin-bottom: 20px; color: #2c3e50;">Select Activity Snippet</h3>';

    for (const [lessonTitle, activities] of Object.entries(coursebookSnippets)) {
        html += `<h4 style="color: #9b59b6; margin: 15px 0 10px 0;">${lessonTitle}</h4>`;
        activities.forEach((activity, index) => {
            html += `
                <div class="snippet-item" data-lesson="${lessonTitle}" data-index="${index}" style="
                    padding: 10px;
                ...
                " onmouseover="this.style.background='#f8f9fa'" onmouseout="this.style.background='white'">
                    <strong>${activity.title}:</strong> ${activity.content.substring(0, 100)}...
                </div>
            `;
        });
    }

    html += '<button onclick="this.closest(\'.snippet-modal\').remove()" style="margin-top: 20px; padding: 10px 20px; background: #e74c3c; color: white; border: none; border-radius: 6px; cursor: pointer;">Close</button>';
    ...
    modalContent.addEventListener('click', function(e) {
        if (e.target.classList.contains('snippet-item')) {
            const lessonTitle = e.target.dataset.lesson;
            const index = parseInt(e.target.dataset.index);
            const snippet = coursebookSnippets[lessonTitle][index];
            activityInput.value = snippet.content;
            modal.remove();
        }
    });
    ...
    activityInputs.forEach(input => {
        ...
        if (!input.nextElementSibling || !input.nextElementSibling.classList.contains('snippet-btn')) {
            ...
            snippetBtn.style.cssText = `
                ...
                transition: all 0.3s;
            `;
            snippetBtn.onmouseover = function() { this.style.background = '#8e44ad'; };
            snippetBtn.onmouseout = function() { this.style.background = '#9b59b6'; };
            snippetBtn.onclick = function() { showSnippetModal(input); };
            input.parentNode.appendChild(snippetBtn);
        }

        // Add image button
        if (!input.parentNode.querySelector('.image-btn')) {
            ...
            imageBtn.style.cssText = `
                ...
                transition: all 0.3s;
            `;
            imageBtn.onmouseover = function() { this.style.background = '#229954'; };
            imageBtn.onmouseout = function() { this.style.background = '#27ae60'; };
...
// Initialize coursebook functionality
document.addEventListener('DOMContentLoaded', function() {
    loadCoursebookSnippets();
    // Add buttons when table is modified
    document.getElementById('stagesTable').addEventListener('DOMNodeInserted', addSnippetButtons);
...
async function uploadImageForActivity(activityInput) {
